...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (b.action === "constancia") {
      ...
      var empId = null, srcId = null;
      if (cKind === "legajo") {
        var erL = await fetch(base + "/rest/v1/employees?sign_token=eq." + encodeURIComponent(token) + "&select=id&limit=1", { headers: headers });
        var rL = await erL.json(); if (Array.isArray(rL) && rL[0]) { empId = rL[0].id; srcId = rL[0].id; }
      } else {
        var tbl = (cKind === "hs") ? "health_safety" : "hr_documents";
        var erR = await fetch(base + "/rest/v1/" + tbl + "?sign_token=eq." + encodeURIComponent(token) + "&select=id,employee_id&limit=1", { headers: headers });
        var rR = await erR.json(); if (Array.isArray(rR) && rR[0]) { empId = rR[0].employee_id; srcId = rR[0].id; }
      }
      ...
      docsArr.push({ id: rid, label: "Firmado", folder: "firmados", title: cTitle, filename: cTitle + ".pdf", url: pdfUrl, signed_pdf: true, source: cKind, source_id: srcId, signed_at: new Date().toISOString() });
